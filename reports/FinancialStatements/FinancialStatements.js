...
async function getLedgerEntries(fromDate, toDate, accounts) {
    ...
    const ledgerEntries = await dataent.db.getAll({
...
async function getAccounts(rootType) {
    let accounts = await dataent.db.getAll({
        doctype: 'Account',
...
async function getFiscalYear() {
    let { fiscalYearStart, fiscalYearEnd } = await dataent.getSingle('AccountingSettings');
    return {
