                    ...
                    <div class='col-6'>${(doc.taxes || []).map(row => {
                        return `<div class='row'>
                                ...
                                <div class='col-6 text-right'>
                                    ${dataent.format(row.amount, 'Currency')}
                                </div>
                            </div>`
                        }).join('')}
                    </div></div>`;
    ...
    links: [
        {
            ...
            action: async form => {
                const payment = await dataent.getNewDoc('Payment');
                ...
                const formModal = await dataent.desk.showFormModal('Payment', payment.name);
            }
        }
    ],
    ...
    listSettings: {
        ...
        getRowHTML(list, data) {
            return `<div class="col-3">${list.getNameHTML(data)}</div>
                    ...
                    <div class="col-4 text-muted text-right">${dataent.format(data.grandTotal, "Currency")}</div>`;
        }
    }
}