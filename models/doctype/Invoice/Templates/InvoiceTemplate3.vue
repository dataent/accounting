    ...
    <div :style="[$.regularFontSize, $.font]" style="font-family: sans-serif;">
        ...
        <div class="row no-gutters mt-5">
            ...
            <div class="col-6 pr-5 text-right">
                ...
                <p :style="$.paraStyle">{{ dataent.format(doc.date, 'Date') }}</p>
            </div>
        </div>
        <div class="row no-gutters mt-5">
            <div class="col-6 text-left pl-5">
                ...
                <table class="table">
                    <tbody>
                        ...
                        <tr v-for="row in doc.items" :key="row.idx">
                            ...
                            <td :style="$.tablePadding" class="text-left">{{ dataent.format(row.rate, 'Currency') }}</td>
                            <td :style="$.tablePadding">{{ row.quantity }}</td>
                            <td :style="$.tablePadding" class="text-right pr-5">{{ dataent.format(row.amount, 'Currency') }}</td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td :style="$.tablePadding" class="text-right pr-5">{{ dataent.format(doc.netTotal, 'Currency') }}</td>
                        </tr>
                        <tr v-for="tax in doc.taxes" :key="tax.name">
                            <td colspan="2" :style="[$.hideBorderTop, $.tablePadding]"></td>
                            <td :style="$.tablePadding" med colspan="2">{{ tax.account.toUpperCase() }} ({{ tax.rate }}%)</td>
                            <td :style="$.tablePadding" class="text-right pr-5">{{ dataent.format(tax.amount, 'Currency') }}</td>
                        </tr>
                        <tr>
                            <td colspan="2" :style="[$.hideBorderTop, $.tablePadding]"></td>
                            <td :style="[$.bold, $.tablePadding, $.mediumFontSize]" colspan="2">TOTAL</td>
                            <td :style="[$.bold, $.tablePadding, $.mediumFontSize]" class="text-right pr-5">{{ dataent.format(doc.grandTotal, 'Currency') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
