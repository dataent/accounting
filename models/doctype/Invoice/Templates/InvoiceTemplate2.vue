    ...
    <div :style="[$.regularFontSize, $.font]" style="font-family: sans-serif;">
        ...
        <div class="row p-5 mt-4">
            ...
            <div class="col-4 text-right">
                ...
                <h2 :style="$.fontColor">{{ dataent.format(doc.grandTotal, 'Currency') }}</h2>
            </div>
        </div>
                ...
                <table class="table table-borderless p-0">
                    ...
                    <tbody>
                        <tr :style="$.showBorderBottom" v-for="row in doc.items" :key="row.idx">
                            ...
                            <td class="text-left pl-0">{{ dataent.format(row.rate, 'Currency') }}</td>
                            <td class="text-left">{{ row.quantity }}</td>
                            <td class="text-right pr-1">{{ dataent.format(row.amount, 'Currency') }}</td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td :style="$.bold" class="text-right pr-1">{{ dataent.format(doc.netTotal, 'Currency') }}</td>
                        </tr>
                        <tr v-for="tax in doc.taxes" :key="tax.name">
                            ...
                            <td :style="$.bold" class="text-right pr-1">{{ dataent.format(tax.amount, 'Currency') }}</td>
                        </tr>
                        <tr>
                            <td colspan="2" :style="$.hideBorderTop" class="text-left pl-1"></td>
                            <td colspan="2" :style="[$.bold, $.fontColor, $.mediumFontSize]" class="text-left pl-0">TOTAL</td>
                            <td :style="[$.bold, $.mediumFontSize]" class="text-right pr-1">{{ dataent.format(doc.grandTotal, 'Currency') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
